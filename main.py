...
##############################################################
# Exchange Associated Functions
#-------------------------------------------------------------

async def updates_sync():
    while True:
        ...
        for account in accounts:
            ...
            has_o_t = int(await has_open_trades(account) == 'True')  # Convert "True"/"False" to 1/0
            has_o_o = int(await has_open_orders(account) == 'True')  # Convert "True"/"False" to 1/0
...
async def fetch_qty(account, symbols=["BTC/USDT:USDT"]):
    ...
    try:
        ...
        for symbol in symbols:
            open_positions = await session.get_positions(symbol=symbol)
        ...
        #    open_orders = session.get_open_orders(symbol=symbol)
...
async def get_diff(step, howMany):

    # Get Klines from Public Streaming Session
    public_session : ExchangePublicStream = ExchangeFactory.get_public_session(testnet=m_testnet)
    ohlcv = await public_session.get_last_klines(symbol="BTC/USDT:USDT", nth=howMany, steps=step)

    # Calculate the timestamp & high_low_diff
    ...
    # Return High - Low difference
    return int_row['high_low_diff']

async def get_max_quantity(account, symbol="BTC/USDT:USDT", entry_limit=2000):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=[symbol])
    print(f"entry_limit is {entry_limit}")

    # Get the available USDT balance
    balance_info = await session.get_wallet_balance(account_type="UNIFIED", coin="USDT")
    ...
    leverage_info = await session.get_positions(symbol=symbol)
    try:
...
async def has_open_orders(account, symbols=["BTC/USDT:USDT"]):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=symbols)
    try:
        for symbol in symbols:
            open_orders = await session.get_open_orders(symbol=symbol)
...
async def get_open_order_ids(account, symbols=["BTC/USDT:USDT"]):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=symbols)
    try:
        order_ids = []
...
async def get_trade_ids(account, symbols=["BTC/USDT:USDT"]):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=symbols)
    try:
        trade_ids = []
...
async def has_open_trades(account, symbols=["BTC/USDT:USDT"]):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=symbols)
    try:
        for symbol in symbols:
            open_positions = await session.get_positions(symbol=symbol)
...
async def place_close_by_order(account, prediction, side, qty):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=["BTC/USDT:USDT"])
    ...
    try:
        close_order_response = await session.place_order(
...
async def cancel_pending_order(account, order_id):
    session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=["BTC/USDT:USDT"])
    cleaned_order_id = clean_order_id(order_id)  # Ensure we clean the order ID

    try:
        cancel_response = await session.cancel_order(
            symbol="BTC/USDT:USDT",
...
async def monitorTrades(account_manager):
    logging.info("Monitoring Accounts")
    for account in account_manager.accounts:
        api_key = account['api_key']
        logging.info(f"API Key: {api_key} - Has Open Trades: {await has_open_trades(account)}")
        logging.info(f"API key: {api_key} - Has Open Orders: {await has_open_orders(account)}")    
...
async def getEntryLimit(account, symbol="BTC/USDT:USDT"):
    public_session: ExchangePublicStream = ExchangeFactory.get_public_session(testnet=m_testnet)
    ticker_data = await public_session.get_last_ticker(symbol=symbol)
...
class AccountManager:
    ...
    async def initialize(self, config_file):
        self.accounts = await self.load_accounts(config_file)

    async def load_accounts(self, config_file):
        ...
        # Initialize missing keys with default values
            ...
            account.setdefault('timestamp', get_utc_current_time())
        ...
        for account in data['accounts']:
            ...
            if r_account == 'NOREC' :
                ...
                await get_open_order_ids(account)
                await get_trade_ids(account)
    ...
    async def find_next_available_account(self):
        logging.info("second entry")
        for account in self.accounts:
            logging.info(f"Checking account {account['api_key']} for open trades/orders")
            if await has_open_trades(account) == "False" and await has_open_orders(account) == "False":
...
class TradeExecutor:
    ...
    async def initialize(self, account_manager, leverage):
        self.leverage = str(leverage)
        self.account_manager = account_manager
        await self.set_leverage_for_all_accounts()

    async def set_leverage_for_all_accounts(self):
        ...
        for account in self.account_manager.accounts:
            if await has_open_trades(account) == "False" and await has_open_orders(account) == "False":
                session : ExchangeWebSocket = ExchangeFactory.get_session(testnet=m_testnet, api_key=account['api_key'], api_secret=account['api_secret'], symbols=["BTC/USDT:USDT"])
                try:
                    leverage_response = await session.set_leverage(
                        symbol="BTC/USDT:USDT", 
    ...
    async def execute_trade_logic(self, account, n_minutes, leverage, spread):
        ...
        if res != "CallAgain":
            ...
            entry_limit = await getEntryLimit(account, symbol="BTC/USDT:USDT")
            ...
            market_irregular= await get_diff(n_minutes,2)
            ...
            if calculated_spread >= spread and market_irregular <= 300 :                
                ...
                try:
                    valid_qty = await get_max_quantity(account, symbol="BTC/USDT:USDT", entry_limit=entry_limit)
...
async def monitor_and_manage_trades(account_manager):
    while True:
...
async def main(exchange, leverage, howMany, n_minutes, spread, config_path):
    ...
    await account_manager.initialize(config_file=config_path)
    ...
    async def process_trades():
        for _ in range(howMany):
            ...
            account = await account_manager.find_next_available_account()
            if account is None:
...
if __name__ == "__main__":
    ...
    asyncio.run(main(args.exchange, args.leverage, args.howMany, args.n_minutes, args.spread, args.config_path))
